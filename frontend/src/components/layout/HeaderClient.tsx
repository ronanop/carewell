...
import { MEGA_COLUMNS } from "@carewell/backend/lib/nav-fallback";
...
export function HeaderClient({
  ...
  return (
    <>
      <header
        className="fixed left-0 right-0 z-50 bg-white shadow-navbar transition-[background,box-shadow,backdrop-filter] duration-300"
      >
        <div className="mx-auto flex h-[60px] max-w-7xl items-center justify-between gap-4 px-4 md:h-[70px] md:px-6">
          <Link
            href="/"
            className="block"
            aria-label="Care Well Medical Centre home"
          >
            <Image
              src="/navbar-logo.png"
              alt="Care Well Medical Centre"
              width={285}
              height={67}
              priority
          ...
          <div className="hidden items-center gap-3 lg:flex">
            {whatsapp && (
              <a
                ...
                aria-label="WhatsApp"
                onClick={() => trackEvent("whatsapp_click", { source: "header-icon", pathname })}
              >
            ...
            <div className="flex items-center justify-between border-b border-surface px-4 py-3">
              {phone && (
                <a
                  ...
                  onClick={() => trackEvent("call_click", { source: "mobile-drawer-top" })}
                >
                  Call now
                </a>
              )}
