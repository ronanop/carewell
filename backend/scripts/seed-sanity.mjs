...
async function seedLegacyServicesFromMap() {
  const p = join(repoRoot(import.meta.url), "db", "seed", "legacy-url-map.json");
  if (!existsSync(p)) {
    console.log("No db/seed/legacy-url-map.json — skip legacy service seed.");
    return;
  }
