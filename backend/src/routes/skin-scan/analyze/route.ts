...
export async function POST(req: Request) {
  ...
  if (typeof imageBase64 !== "string" || typeof mimeType !== "string") {
    return Response.json({ ok: false, error: "Expected imageBase64 and mimeType strings." }, { status: 400 });
  }
  ...
  if (imageBase64.length > MAX_BASE64_LENGTH || imageBase64.length < 100) {
    return Response.json({ ok: false, error: "Image payload size is invalid." }, { status: 400 });
  }
